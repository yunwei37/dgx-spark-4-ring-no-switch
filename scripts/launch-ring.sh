...
usage() {
  cat <<'EOF'
Usage: launch-ring.sh [--dry-run|--stop] <profile.sh>
...
Required environment:
  ...
  SOCKET_IFNAME    Management interface used for rendezvous/control traffic.
  ...
  VLLM_API_KEY     API key passed directly to each container.
...
launch_rank() {
  ...
  local command=(
    ...
    -v "$CACHE_HOST_PATH:/root/.cache/vllm"
  )
  local item
  ...
  command+=(
    "$IMAGE" vllm "${VLLM_PROFILE_ARGS[@]}"
    --nnodes 4
    --node-rank "$rank"
    --master-addr "$MASTER_ADDR"
    --master-port "$MASTER_PORT"
  )
  [[ $rank -ne 0 ]] && command+=(--headless)
