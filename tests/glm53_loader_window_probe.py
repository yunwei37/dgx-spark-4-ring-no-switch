...
print(json.dumps({"phase":"start","mode":MODE,"source_sha256":hashlib.sha256(source).hexdigest(),"files":files,"file_bytes":sum(pathlib.Path(f).stat().st_size for f in paths),"mmap":True,"prefetch":False,"drop_cache":False,"cuda_cap":4*1024**3}),flush=True)
iterator=(buffered_multi_thread_safetensors_weights_iterator(paths,max_workers=8) if MODE=="multithread8" else safetensors_weights_iterator(paths))
try:
