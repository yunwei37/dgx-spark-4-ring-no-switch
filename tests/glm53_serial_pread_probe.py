...
print(json.dumps({"phase":"start","mode":MODE,"run":RUN,"source_sha256":hashlib.sha256(source).hexdigest(),"files":files,"file_bytes":sum(pathlib.Path(f).stat().st_size for f in paths),"backend":MODE,"safe_open_signature":api_signature,"torch_version":torch.__version__,"safetensors_version":native_safetensors.__version__,"iterator":"native_serial","consumer_keeps_previous_cpu_tensor":True,"sample_interval_seconds":.05,"prefetch":False,"drop_cache":False,"cuda_cap":4*1024**3}),flush=True)
iterator=safetensors_weights_iterator(paths)
try:
 for name,param in iterator:
  ...
  del raw,g  # Match a normal consumer loop retaining param until next yield.
...
r={"phase":"result","mode":MODE,"run":RUN,"seconds":time.monotonic()-start,"tensors":count,"tensor_bytes":total,"edge_digest":digest.hexdigest(),"source_sha256":hashlib.sha256(source).hexdigest(),"cuda_peak_allocated":torch.cuda.max_memory_allocated(),"cuda_peak_reserved":torch.cuda.max_memory_reserved(),"mem_available_start":baseline["MemAvailable"],"mem_available_min":min(x["MemAvailable"] for x in samples),"process_peaks":{k:max(x[k] for x in samples if x[k] is not None) for k in ["VmRSS","RssAnon","RssFile","VmLck","VmPin"]},"normal_free_pages_min":min((x["Normal_free_pages"] for x in samples if x["Normal_free_pages"] is not None),default=None),"samples":len(samples),"iterator":"native_serial","consumer_keeps_previous_cpu_tensor":True,"scope":"real first-ten-shard CPU-to-GPU copies with ordinary for-loop input lifetime, not complete model or inference; cache order uncontrolled"}
